...
void Read_n(int *n_p, int *local_n_p, int my_rank, int comm_sz,
            MPI_Comm comm);
void Read_scalar(double *n_s, int my_rank, int comm_sz,
            MPI_Comm comm);
...
void Print_vector(double local_b[], int local_n, int n, char title[],
                  int my_rank, MPI_Comm comm);
void Parallel_vector_sum(double local_x[], double local_y[], int local_n, double scalar);
...
int main(void)
{
    ...
    Parallel_vector_sum(local_x, local_y, local_n, scalar);
    Print_vector(local_x, local_n, n, "The sum is", my_rank, comm);
    Print_vector(local_y, local_n, n, "The sum is", my_rank, comm);
 ...
 *            comm:       communicator containing all the processes
 *                        calling Read_scalar
 * Out args:  n_s:        global value of n
 *            local_n_s:  local value of n = n/comm_sz
 *
 * Errors:    n should be positive and evenly divisible by comm_sz
 */
void Read_scalar(
    ...
    MPI_Bcast(n_s, 1, MPI_DOUBLE, 0, comm);
} /* Read_scalar */
    ...
    double scalar)
{
    ...
    for (local_i = 0; local_i < local_n; local_i++) {
        local_x[local_i] = local_x[local_i] * scalar;
        local_y[local_i] = local_y[local_i] * scalar;
    }
} /* Parallel_vector_sum */
